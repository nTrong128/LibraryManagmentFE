            ...
            <TableRow v-for="application in borrowApplications" :key="application.id">
              ...
              <TableCell>
                <div class="flex space-x-2">
                  <Button v-if="application.status === 'Pending'" size="sm" @click="approveApplication(application.id)">
                    Approve
                  </Button>
                  <Button v-if="application.status === 'Pending'" size="sm" variant="destructive" @click="rejectApplication(application.id)">
                    Reject
                  </Button>
                </div>
              </TableCell>
    ...
    <Dialog :open="showConfirmDialog" @update:open="showConfirmDialog = false">
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button class="" @click="confirmDialog(true)">
